...
REM - TURN ON FIREWALL - WORKS
netsh advfirewall set allprofiles state on

REM - SET AUDIT POLICY - WORKS
...
REM - STOP FILE SHARING - WORKS
netsh advfirewall firewall set rule group="File and Printer Sharing" new enable=No

REM - STOPS AND DISABLES SERVICES - WORKS
::DNS Client
...
REM - ENABLE SECURE LOG ON - WORKS
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" /v DisableCAD /t REG_DWORD /d 0 /f

REM - CALLS ACCOUNT PROPERTIES MANAGEMENT PS1 FILE - WORKS
cls
echo Changing Account Properties
powershell -noprofile -command "&{ start-process powershell -ArgumentList '-noprofile -file %~D0Account_Properties_Management.ps1' -verb RunAs}"
timeout /T 30 

REM - CALLS PYTHON SCRIPT TO CHANGE ALL NON_ADMIN PASSWORDS - WORKS
echo.
echo.
echo Changine Non Admin Passwords
%~D0Python\python.exe %~D0User_Script.py
echo.
echo Passwords Changed!
timeout /T 3 /nobreak>nul

REM - IMPORT SECURITY POLICY CONFIG FILE - WORKS
echo.
echo.
echo Copying and Importing Security Policy
echo.
timeout /T 1 /nobreak>nul
copy %~D0secconfig.cfg C:\secconfig.cfg
timeout /T 1 /nobreak>nul
...
del "%TEMP%\*.*" /s /f /q
timeout /T 2 /nobreak>nul
REM set SRVC_LIST=(RemoteAccess Telephony tlntsvr p2pimsvc simptcp fax msftpsvc)
REM 	for %%i in %HITHERE% do net stop %%i
REM 	for %%i in %HITHERE% sc config %%i start= disabled
timeout /T 2 /nobreak>nul
